...
<Note>
  **Placement:** Call `addUserInfo` after successful login or signup, not during component initialization. If you include an `email` field in `userProperties`, the SDK will automatically use it as the user ID.
</Note>
