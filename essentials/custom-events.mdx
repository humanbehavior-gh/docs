...
```javascript
// Page views (in addition to automatic tracking)
await tracker.customEvent('page_viewed', {
  page: '/pricing',
  referrer: '/features',
  userType: 'new'
});
...
```javascript
// Track custom navigation (in addition to automatic tracking)
function trackNavigation(from, to, type = 'custom') {
  tracker.customEvent('navigation', {
    from: from,
    to: to,
    type: type,
    timestamp: Date.now()
...
// Usage
trackNavigation('/home', '/products', 'menu_click');
```
...
window.addEventListener('beforeunload', async () => {
  ...
  await tracker.customEvent('page_time_spent', {
    page: window.location.pathname,
    timeSpent: timeSpent,
