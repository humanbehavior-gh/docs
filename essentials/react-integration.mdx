...
// Usage
<TrackedLink 
  eventName="feature_link_clicked"
  eventProperties={{ feature: 'analytics' }}
...
The React integration automatically tracks route changes:
...
function useRouteTracking() {
  ...
  useEffect(() => {
    const trackRouteChange = async () => {
      await tracker.customEvent('route_changed', {
