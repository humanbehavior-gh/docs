---
title: 'Next.js Setup'
description: 'Complete setup guide for HumanBehavior SDK with Next.js applications'
---

HumanBehavior SDK is fully compatible with Next.js and provides seamless integration for both App Router and Pages Router. This guide covers everything you need to set up session recording in your Next.js application.
...
NEXT_PUBLIC_HUMANBEHAVIOR_INGESTION_URL=https://ingestion.humanbehavior.ai
```
...
export function createTracker() {
  return new HumanBehaviorTracker(
    ...
    process.env.NEXT_PUBLIC_HUMANBEHAVIOR_INGESTION_URL || 'https://ingestion.humanbehavior.ai'
  );
}
```
...
```bash
# Netlify
NEXT_PUBLIC_HUMANBEHAVIOR_API_KEY=your-key
...
## ⚠️ Important: Using the Provider in Next.js App Router

<Warning>
If you are using the Next.js App Router (the `/app` directory), **React components are server components by default**. The `HumanBehaviorProvider` (and any component using React context or hooks) must be used in a <strong>client component</strong>.<br/><br/>
