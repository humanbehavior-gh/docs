...
```javascript
// Session persists when navigating between pages on the same domain
// Example: /home → /products → /checkout
// All interactions are part of the same session
```
...
await tracker.trackNavigationEvent('custom', '/home', '/products');
...
console.log('Connection blocked:', connectionInfo.blocked);
