...
4. **URL Tracking**: Entry URL and navigation URLs are automatically tracked
...
The SDK automatically captures URL information:

```javascript
// Entry URL is captured when session starts
// All URL changes are tracked during the session
// End URL is captured when session completes
```

### URL Data Structure

Each session includes comprehensive URL data:
...
```javascript
{
  ...
  endURL: 'https://example.com/checkout',
...
```javascript
{
  ...
  entryURL: 'https://example.com/start',
  endURL: 'https://example.com/checkout',
  uniqueURLsVisitedCount: 3
}
```

## Enhanced Session Metrics

The SDK automatically tracks detailed interaction metrics:

### Interaction Counts

- **`clickCount`**: Number of button and link clicks
- **`keypressCount`**: Number of keyboard interactions in input fields
- **`mouseActivityCount`**: Total mouse movements and interactions
- **`pageviewCount`**: Number of page views during the session
- **`autocaptureCount`**: Number of automatically captured interactions

### URL Tracking

- **`entryURL`**: The first URL visited in the session
- **`endURL`**: The last URL visited before session completion
- **`allURLs`**: Array of all URLs visited during the session
- **`uniqueURLsVisitedCount`**: Number of unique pages visited
...
const isActive = tracker.isInitialized();
...
<Note>
  **Version 0.0.8**: Enhanced URL tracking automatically captures entry URLs, navigation paths, and end URLs for comprehensive session analysis.
</Note>
