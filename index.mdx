...
<CardGroup cols={2}>
  ...
  <Card
    ...
    href="/essentials/session-tracking"
  >
    Automatic SPA navigation tracking and URL change detection
  </Card>
</CardGroup>
...
The SDK includes robust anti-blocking measures:

- **Automatic fallback** from `sendBeacon` to `fetch`
- **Connection testing** and status reporting
- **Graceful error handling** for ad blockers
- **Event queuing** during network issues
...
<Note>
  Version 0.0.9 includes enhanced session persistence, improved navigation tracking, and better anti-blocking measures for a more robust user experience.
</Note>
