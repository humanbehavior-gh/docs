...
interface HumanBehaviorInterface {
  ...
  addUserInfo: ({ userId, userProperties }: { userId: string; userProperties: Record<string, any> }) => Promise<void>;
  start: () => Promise<void>;
  stop: () => Promise<void>;
...
Identifies a user and adds their properties. This updates the current user ID and stores user information.
