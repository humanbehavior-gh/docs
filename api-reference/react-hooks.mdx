...
interface HumanBehaviorInterface {
  ...
  identifyUser: (userId: string, userProperties: Record<string, any>) => Promise<string>;
  start: () => Promise<void>;
  stop: () => Promise<void>;
...
Identifies and tracks users across sessions with persistent user IDs and properties.

**Parameters:**
- `userId` (string, required): Unique user identifier (should be stable across sessions)
...
const userId = await tracker.identifyUser('user-123', {
...
function OptimizedComponent() {
  ...
  useEffect(() => {
          tracker.identifyUser('user-123', userInfo);
  }, [tracker, userInfo]);
