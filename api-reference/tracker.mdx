...
Identify a user and add their properties. This updates the current user ID and stores user information.
...
**Note:** User ID management is mostly automatic. You only need this when explicitly identifying a user (like during login).

### `auth(authFields)`

Check if the current user matches an existing user in your system.
...
**Example:**
```javascript
// Check if current user matches existing user by email
await tracker.auth(['email']);
```

**Note:** This is typically used when you want to link anonymous sessions to existing users.
